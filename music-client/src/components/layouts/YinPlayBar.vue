...
<template>
  <div class="play-bar" :class="{ show: !toggle }">
    <div class="fold" :class="{ turn: toggle }">
      <YinIcon :icon="iconList.ZHEDIE" @click="toggle = !toggle" />
    </div>
    <!-- 播放进度 -->
    ...
    <div class="control-box">
      ...
      <div class="song-ctr">
        ...
        <!-- 上一首 -->
        <YinIcon class="yin-play-show" :icon="iconList.SHANGYISHOU" @click="prev" />
        <!-- 播放 -->
        <YinIcon :icon="playBtnIcon" @click="togglePlay" />
        <!-- 下一首 -->
        <YinIcon class="yin-play-show" :icon="iconList.XIAYISHOU" @click="next" />
        <!-- 音量 -->
        <el-dropdown class="yin-play-show" trigger="click">
          <YinIcon v-if="volume !== 0" :icon="iconList.YINLIANG" />
          <YinIcon v-else :icon="iconList.JINGYIN" />
          <template #dropdown>
            <el-dropdown-menu>
              <el-slider
      ...
      <div class="song-ctr song-edit">
        ...
        />
        <!-- 歌曲列表 -->
        <YinIcon :icon="iconList.LIEBIAO" @click="changeAside" />
      </div>
    </div>
  </div>
</template>
